...
void Generacija(int Pas) {
    for (int fileIndex = 1000; fileIndex <= 10000000; fileIndex *= 10) {
        ...
        std::deque<Studentas> highGrades;
        std::deque<Studentas> lowGrades;
        ...
        if (Pas == 3)
        {
            ...
            string line;
            getline(inputFile, line); // Skip the header line
            deque<Studentas> students; // Declare the students list
            while (getline(inputFile, line)) {
                istringstream iss(line);
                Studentas student;
                iss >> student.vardas >> student.pavarde;
            ...
            auto startSort = high_resolution_clock::now();
            deque<Studentas> highGradeStudents;
            deque<Studentas> lowGradeStudents;
